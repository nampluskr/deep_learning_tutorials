...
class EfficientADTrainer(BaseTrainer):
    ...
    def run_epoch(self, loader, epoch, num_epochs, mode="train"):
        if mode == "train":
        ...
        results = {"loss": 0.0, "st": 0.0, "ae": 0.0, "stae": 0.0}
        num_images = 0

        with tqdm(loader, ascii=True, leave=False) as pbar:
            for batch in pbar:
                pbar.set_description(f" [{epoch:3d}/{num_epochs}] {mode.upper()}")
                ...
                results["st"]   += loss_st.item() * batch_size
                results["ae"]   += loss_ae.item() * batch_size
                results["stae"] += loss_stae.item() * batch_size

                pbar.set_postfix({
                    "loss": f"{results['loss']/num_images:.3}",
                    "st":   f"{results['st']/num_images:.3}",
                    "ae":   f"{results['ae']/num_images:.3f}",
                    "stae": f"{results['stae']/num_images:.3f}",
                })
        return {k: v / num_images for k, v in results.items()}

    def fit(self, train_loader, num_epochs, valid_loader=None, weight_path=None):
        print("\n > Start training...\n")
        history = {name: [] for name in ['loss', 'st', 'ae', 'stae'] + list(self.metrics)}
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if valid_loader is not None:
                ...
                # valid_results = self.run_epoch(train_loader, epoch, num_epochs, mode="valid")
