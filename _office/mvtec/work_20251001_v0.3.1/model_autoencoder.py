...
class AutoEncoderTrainer(BaseTrainer):
    def __init__(self, model, optimizer=None, loss_fn=None, metrics=None, device=None):
        ...
        self.loss_fn = loss_fn or AECombinedLoss(mse_weight=0.7, ssim_weight=0.3)
        self.metrics = metrics or {"ssim": SSIMMetric(data_range=2.0, reduction="elementwise_mean")}

        if isinstance(self.loss_fn, nn.Module):
            self.loss_fn.to(self.device)

        for metric_name in self.metrics:
            if isinstance(self.metrics[metric_name], nn.Module):
                self.metrics[metric_name].to(self.device)
    ...
    def run_epoch(self, loader, epoch, num_epochs, mode='train'):
        ...
        with tqdm(loader, leave=False, ascii=True) as pbar:
            for batch in pbar:
                ...
                with torch.no_grad():
                    for name, metric_fn in self.metrics.items():
                        score = metric_fn(images, recon).item()
                        results[name] += metric_fn(images, recon).item() * batch_size

                pbar.set_postfix({n: f"{v/num_images:.3f}" for n, v in results.items()})
        return {name: v / num_images for name, v in results.items()}
