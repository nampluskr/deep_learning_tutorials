...
class BaseTrainer(ABC):
    ...
    @torch.no_grad()
    def test_feature_based(self, test_loader, output_dir=None, show_image=False, img_prefix="img",
        skip_normal=False, skip_anomaly=False, num_max=-1):
        """Test method for feature-based models (STFPM, EfficientAD, Manual)."""
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
        ...
        for batch in test_loader:
            ...
            # Extract teacher and student features based on model type
            if len(model_output) == 2:
                # STFPM, Manual: (teacher_features, student_features)
                teacher_features, student_features = model_output
                reconstructed = None
            elif len(model_output) == 3:
                # EfficientAD: (reconstructed, teacher_features, student_features)
                reconstructed, teacher_features, student_features = model_output
            else:
                raise ValueError(f"Unexpected model output format: {len(model_output)} elements")
            ...
            for i in range(images.size(0)):
                ...
                # Process teacher features for visualization
                teacher_feat = teacher_features[i] if isinstance(teacher_features, torch.Tensor) else teacher_features[0][i]
                if teacher_feat.dim() == 3:  # (C, H, W)
                    teacher_feat_vis = torch.mean(teacher_feat, dim=0)  # Average across channels
                else:
                    teacher_feat_vis = teacher_feat
                teacher_feat_vis = teacher_feat_vis.cpu().numpy()
                teacher_feat_vis = (teacher_feat_vis - teacher_feat_vis.min()) / (teacher_feat_vis.max() - teacher_feat_vis.min() + 1e-8)

                # Process student features for visualization
                student_feat = student_features[i] if isinstance(student_features, torch.Tensor) else student_features[0][i]
                if student_feat.dim() == 3:  # (C, H, W)
                    student_feat_vis = torch.mean(student_feat, dim=0)  # Average across channels
                else:
                    student_feat_vis = student_feat
                student_feat_vis = student_feat_vis.cpu().numpy()
                student_feat_vis = (student_feat_vis - student_feat_vis.min()) / (student_feat_vis.max() - student_feat_vis.min() + 1e-8)
                ...
                if binary_mask is not None:
                    ...
                    titles = [f"Original: {label}", "Mask", "Teacher Feature", "Student Feature", f"Anomaly: {score:.4f}"]
                    images_vis = [original, binary_mask, teacher_feat_vis, student_feat_vis, anomaly_map_vis]
                    cmaps = [None, "gray", "viridis", "viridis", "jet"]
                else:
                    fig, axes = plt.subplots(1, 4, figsize=(12, 3))
                    titles = [f"Original: {label}", "Teacher Feature", "Student Feature", f"Anomaly: {score:.4f}"]
                    images_vis = [original, teacher_feat_vis, student_feat_vis, anomaly_map_vis]
...
class STFPMTrainer(BaseTrainer):
    def __init__(self, model, optimizer=None, loss_fn=None, metrics=None, device=None):
        from model_stfpm import STFPMLoss, STFPMMetric

        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = model.to(self.device)
        self.optimizer = optimizer or optim.AdamW(
            [p for p in model.parameters() if p.requires_grad], lr=1e-4, weight_decay=1e-5)
        self.loss_fn = loss_fn or STFPMLoss()
        self.metrics = metrics or {'similarity': STFPMMetric()}
