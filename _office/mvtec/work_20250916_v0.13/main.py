...
def run_autoencoder(num_epochs=10, output_dir=None, model_type="ae"):
    ...
    if output_dir is not None:
        ...
        weight_name = f"{CATEGORY}_{model_type}_epochs-{num_epochs}.pth"
        trainer.save_model(os.path.join(output_dir, weight_name))
...
def run_stfpm(backbone="resnet18", num_epochs=10, output_dir=None, model_type="stfpm"):
    ...
    train_loader, test_loader = get_dataloaders(
        ...
        category=CATEGORY, batch_size=4, img_size=256)
    device = get_device()
    model = STFPMModel(layers=["layer1", "layer2", "layer3"], backbone=backbone).to(device)
    show_model_info(model)

    loss_fn = STFPMLoss()
    metrics = {"feat_sim": FeatureSimilarityMetric()}
    optimizer = optim.Adam(model.student_model.parameters(), lr=1e-4, weight_decay=1e-3)
...
if __name__ == "__main__":
    ...
    for category in ["carpet"]:
        OUTPUT_DIR = os.path.join("./experiments", category)
        CATEGORY = category
        ...
        # run_stfpm(backbone="resnet18", num_epochs=50, output_dir=OUTPUT_DIR, model_type="stfpm-resnet18")
        # run_stfpm(backbone="resnet50", num_epochs=50, output_dir=OUTPUT_DIR, model_type="stfpm-resnet50")
        run_efficientad(model_size="small", num_epochs=50, output_dir=OUTPUT_DIR, model_type="efficientad-small")
        # run_efficientad(model_size="medium", num_epochs=50, output_dir=OUTPUT_DIR, model_type="efficientad-medium")
