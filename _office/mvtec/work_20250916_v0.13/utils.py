...
def model_test(model, test_loader, device):
    for inputs in test_loader:
        ...
        predictions = model.evaluate(images)

        names = inputs['name']        
        images = denormalize_imagenet(images).cpu().numpy()
        scores = predictions['pred_score'].detach().cpu().numpy().squeeze(axis=-1)
        masks = inputs['mask'].cpu().numpy()
        anomaly_maps = predictions['anomaly_map'].detach().cpu().numpy().squeeze(1)
        ...
        for i in range(batch_size):
            ...
            im = axes[i, 2].imshow(anomaly_maps[i], cmap='hot', vmin=0, vmax=anomaly_maps[i].max())
            axes[i, 2].set_title(f"Anomaly ({scores[i]:.3e})")
            axes[i, 2].axis('off')
