...
from models.model_ae import UNetAE, AELoss, AECombinedLoss
...
def build_dataset(dataset_type, **dataset_params):
    dataset_type = dataset_type.lower()
    if dataset_type not in DATASET_REGISTRY:
        available_datasets = list(DATASET_REGISTRY.keys())
        raise ValueError(f"Unknown dataset: {dataset_type}. Available datasets: {available_datasets}")
...
def build_trainer(trainer_type, modeler, **trainer_params):
...
def _get_dataloader_params(model_type, dataset_type, base_params):
...
def run_experiment(dataset_type, model_type, config):
    dataset_loader_class = DATASET_REGISTRY.get(dataset_type.lower())
    ...
    dataloader_params = _get_dataloader_params(model_type, dataset_type, base_dataloader_params)
    ...
    print(f"  Training {model_type} on {dataset_type} for {epochs} epochs...")
