...
class KCenterGreedy:
    ...
    def select_coreset_idxs(self, selected_idxs: list[int] | None = None) -> list[int]:
        ...
        for _ in tqdm(range(self.coreset_size), desc="Selecting Coreset Indices.", ascii=True, leave=False):
...
class PatchCore(DynamicBufferMixin, nn.Module):
    def __init__(self, layers, backbone="wide_resnet50_2", pre_trained=True, num_neighbors=9):
        super().__init__()
        self.tiler = None
        ...
        self.feature_extractor = TimmFeatureExtractor(backbone=self.backbone,
            pre_trained=pre_trained, layers=self.layers).eval()
...
class PatchCoreTrainer(BaseTrainer):
    ...
    def fit(self, train_loader, num_epochs=None, valid_loader=None, weight_path=None):
        print("\n > Start PatchCore training... (Embedding extraction only)")
    ...
    def _extract_embeddings(self, loader):
        ...
        for batch in pbar:
            ...
            _ = self.model(imgs)  # embedding_store에 자동 누적

    @torch.no_grad()
    def _build_memory_bank(self):
        if self.memory_built:
            return
        print("\n[INFO] Building PatchCore memory bank via Coreset sampling...")
    ...
    def save_model(self, weight_path):
        if weight_path is not None:
            ...
            ckpt = {"memory_bank": self.model.memory_bank.cpu()}
    ...
    def load_model(self, weight_path):
        ...
        self.memory_built = True
        print(f"[INFO] Loaded PatchCore memory bank from: {weight_path}")
