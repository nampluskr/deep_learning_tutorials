        ...
        if isinstance(backbone, nn.Module):
            ...
            if os.path.exists(local_weights_path):
                ...
                try:
                    ...
                    print(f"Successfully loaded local weights for {backbone}")
                except Exception as e:
                    logger.warning(f"Failed to load local weights: {e}. Using random initialization.")
                    print(f"Failed to load local weights: {e}. Using random initialization.")
            else:
                logger.warning(f"Local weights not found at {local_weights_path}")
                print(f"Local weights not found at {local_weights_path}")
                if pre_trained:
                    logger.warning("Using random initialization instead of pretrained weights.")
                    print("Using random initialization instead of pretrained weights.")
