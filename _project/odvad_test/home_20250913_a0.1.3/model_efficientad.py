...
def load_teacher_weights(model, model_size: str, device: str = "cpu"):
    ...
    try:
        ...
        logger.warning(f"Failed to load teacher weights from {weights_path}: {e}")
...
class EfficientADModel(nn.Module):
    """EfficientAd model integrated with anomaly detection framework."""
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Framework-compatible forward pass for training."""
        student_output, distance_st = self.compute_student_teacher_distance(x)
        
        # For training mode, return losses
        if self.training:
            return self.compute_losses(x, None, distance_st)
        
        # For evaluation mode, return reconstructed image and features (compatibility)
        with torch.no_grad():
            ae_output = self.ae(x, x.shape[-2:])
            
        return ae_output, student_output, distance_st

    def evaluate(self, x: torch.Tensor) -> dict:
        """Framework-compatible evaluation method."""
        student_output, distance_st = self.compute_student_teacher_distance(x)
        map_st, map_stae = self.compute_maps(x, student_output, distance_st, normalize=True)
    ...
    def compute_losses(
        ...
        if len(distance_st_flat) > 0:
            d_hard = torch.quantile(distance_st_flat, 0.99) # 0.999 → 0.99
        ...
        if self.use_imagenet_penalty:
            ...
            loss_st = loss_hard + 0.1 * loss_penalty  # 가중치 추가
        else:
            loss_st = loss_hard
        ...
        distance_ae = torch.mean(torch.pow(teacher_output_aug - ae_output_aug, 2))
        distance_stae = torch.mean(torch.pow(ae_output_aug - student_output_ae_aug, 2))

        loss_ae = distance_ae
        loss_stae = distance_stae
...
class EfficientADLoss(nn.Module):
    """EfficientAD loss function for framework compatibility."""
    
    def __init__(self):
        super().__init__()
    
    def forward(self, loss_st: torch.Tensor, loss_ae: torch.Tensor, loss_stae: torch.Tensor) -> torch.Tensor:
        """Combine all EfficientAD losses."""
        return loss_st + loss_ae + loss_stae


class EfficientADMetric(nn.Module):
    """EfficientAD metric for feature similarity."""
    
    def __init__(self):
        super().__init__()
    
    def forward(self, student_output: torch.Tensor, distance_st: torch.Tensor) -> float:
        """Compute average distance between teacher and student features."""
        return torch.mean(distance_st).item()
