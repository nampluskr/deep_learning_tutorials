#####################################################################
# anomalib/src/anomalib/models/image/dinomaly/components/dinov2_loader.py
#####################################################################
...
dinov2_vits14_pretrain
           myspace/NAMU/backbones/dinov2_vits14_pretrain.pth
/home/namu/myspace/NAMU/backbones/dinov2_vits14_pretrain.pth
"""
...
class DinoV2Loader:
        ...
        if not weight_path.exists():
            if self.offline:
                raise FileNotFoundError(
                    f"[offline mode] Weight file not found: {weight_path}\n"
                    f"Please download manually and place under '{self.cache_dir}'."
                )
            else:
                raise NotImplementedError(
                    f"Online download not implemented. Please manually download {model_name} weights."
                )
    ...
    def _download_weights(self, model_type: str, architecture: str, patch_size: int) -> None:
        ...
        if self.offline:
            weight_path = self._get_weight_path(model_type, architecture, patch_size)
            raise FileNotFoundError(
                f"[offline mode] Required weight file not found: {weight_path}\n"
                f"Please download it manually (see README) and place it under '{self.cache_dir}'."
            )
