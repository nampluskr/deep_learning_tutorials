...
def imagenet_norm_batch(x: torch.Tensor):
...
def reduce_tensor_elems(tensor: torch.Tensor, m: int = 2**24):
...
class EfficientAD(nn.Module):
    def __init__(self, teacher_out_channels=384, model_size="small", image_shape=(256, 256), 
...
class EfficientADTrainer(BaseTrainer):
    ...
    def prepare_imagenette_data(self, image_shape):
        ...
        imagenet_dir = "/mnt/d/backbones/imagenette2"
    ...
    def on_train_start(self, train_loader):
        if train_loader.batch_size != 1:
            raise ValueError("train batch size for EfficientAd should be 1.")
    ...
    def train_step(self, batch):
        try:
            imagenet = next(self.imagenet_iterator)[0].to(self.device)
        except StopIteration:
            self.imagenet_iterator = iter(self.imagenet_loader)
            imagenet = next(self.imagenet_iterator)[0].to(self.device)

        images = batch["image"].to(self.device)
        loss_st, loss_ae, loss_stae = self.model(batch=images, batch_imagenet=imagenet)
        loss = loss_st + loss_ae + loss_stae

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return {"loss": loss.item(), "st": loss_st.item(), "ae": loss_ae.item(), "stae": loss_stae.item()}
