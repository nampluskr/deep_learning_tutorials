...
class DSRTrainer(BaseTrainer):
    def __init__(self, model, optimizer=None, loss_fn=None, metrics=None, device=None):
        ...
        self.perlin_generator = PerlinAnomalyGenerator(
            anomaly_source_path=None,  # DTD 없이 perlin noise만 사용
    ...
    def prepare_pretrained_model() -> Path:
        pretrained_models_dir = Path("/mnt/d/backbones/")
    ...
    def on_fit_start(self):
        ...
        print(f" > Phase 1: epochs 1-{self.second_phase_epoch}")
        print(f" > Phase 2: epochs {self.second_phase_epoch+1}-{self.num_epochs}\n")

    def on_train_start(self, train_loader):
        self.model.train()
        if self.epoch == 1:
            ckpt_path = self.prepare_pretrained_model()

            if ckpt_path.is_file():
                self.model.load_pretrained_discrete_model_weights(ckpt_path, self.device)
                print(f" > Loaded pretrained weights from {ckpt_path}\n")
            else:
                print(f" > Warning: Pretrained weights not found at {ckpt_path}\n")

    def on_epoch_start(self):
        super().on_epoch_start()
        if self.epoch == self.second_phase_epoch + 1:
            self.is_phase2 = True
            print(f"\n > [Phase 2] Now training upsampling module (epoch {self.epoch}/{self.num_epochs})\n")
