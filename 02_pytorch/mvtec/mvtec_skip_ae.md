...
class ImprovedEncoder(nn.Module):
    ...
    def forward(self, x):
        f1 = self.conv1(x)      # 128 x 128 x 32
        f2 = self.conv2(f1)     # 64 x 64 x 64
        f3 = self.conv3(f2)     # 32 x 32 x 128
        f4 = self.conv4(f3)     # 16 x 16 x 256
        f5 = self.conv5(f4)     # 8 x 8 x 512

        pooled = self.pool(f5)
        pooled = pooled.view(pooled.size(0), -1)
        latent = self.fc(pooled)
        return latent, [f1, f2, f3, f4, f5]
...
class ImprovedDecoder(nn.Module):
    def __init__(self, out_channels=3, latent_dim=512):
        ...
        self.deconv1 = DeconvBlock(512 + 512, 256)  # Skip from conv5
        self.deconv2 = DeconvBlock(256 + 256, 128)  # Skip from conv4
        self.deconv3 = DeconvBlock(128 + 128, 64)   # Skip from conv3
        self.deconv4 = DeconvBlock(64 + 64, 32)     # Skip from conv2
    ...
    def forward(self, latent, skip_features=None):
        ...
        x = torch.cat([x, skip_features[4]], dim=1)  # Concat with conv5
        x = self.deconv1(x)     # 16 x 16 x 256

        x = torch.cat([x, skip_features[3]], dim=1)  # Concat with conv4
        x = self.deconv2(x)     # 32 x 32 x 128

        x = torch.cat([x, skip_features[2]], dim=1)  # Concat with conv3
        x = self.deconv3(x)     # 64 x 64 x 64

        x = torch.cat([x, skip_features[1]], dim=1)  # Concat with conv2
        x = self.deconv4(x)     # 128 x 128 x 32

        reconstructed = self.final_conv(x)  # 256x256x3
...
if __name__ == "__main__":
    ...
    encoder = ImprovedEncoder(in_channels=3, latent_dim=512)
    decoder = ImprovedDecoder(out_channels=3, latent_dim=512)
    model = ImprovedAutoEncoder(encoder, decoder).to(device)
