...
class VanillaAE(nn.Module):
    ...
    def train_step(self, data, optimizer, loss_fn, metrics, device):
        inputs = targets = data["image"].to(device)
...
class VAE(nn.Module):
    ...
    def train_step(self, data, optimizer, loss_fn, metrics, device):
        ...
        optimizer.zero_grad()
        outputs = self.forward(inputs)
        preds, mu, logvar = outputs['reconstructed'], outputs['mu'], outputs['logvar']
        loss = loss_fn(preds, targets, mu, logvar)
        loss.backward()
        optimizer.step()
        ...
        with torch.no_grad():
            for metric_name, metric_fn in metrics.items():
                if metric_name == 'vae':
                    results[metric_name] = float(metric_fn(preds, targets, mu, logvar))
                else:
                    results[metric_name] = float(metric_fn(preds, targets))
        return results
    ...
    def validate_step(self, data, loss_fn, metrics, device):
        ...
        with torch.no_grad():
            for metric_name, metric_fn in metrics.items():
                if metric_name == 'vae':
                    results[metric_name] = metric_fn(preds, targets, mu, logvar).item()
                else:
