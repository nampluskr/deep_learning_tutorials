...
def train_batch(model, batch_data, optimizer, loss_fn, metrics={}):
    """General training function for a single batch using model's train_step"""
    if hasattr(model, 'train_step'):
        # Use model's own train_step method
        return model.train_step(batch_data, optimizer, loss_fn, metrics)
    else:
...
def validate_batch(model, batch_data, loss_fn, metrics={}):
    """General validation function for a single batch using model's validate_step"""
    if hasattr(model, 'validate_step'):
        # Use model's own validate_step method
        return model.validate_step(batch_data, loss_fn, metrics)
    else:
        # Fallback to original implementation
        device = next(model.parameters()).device
        model.eval()
...
class Trainer:
    """Training process manager for anomaly detection models
    
    This class manages the complete training workflow including model training,
    validation, optimization, scheduling, and early stopping. It acts as a 
    coordinator between the Modeler and training infrastructure components.
    """

    def __init__(self, modeler, optimizer, scheduler=None, early_stopping=None, logger=None):
        self.modeler = modeler
        self.optimizer = optimizer
        self.scheduler = scheduler
        self.early_stopping = early_stopping
        self.logger = logger
        self.device = next(modeler.model.parameters()).device

        # Initialize history tracking
        self.history = {'loss': []}
        self.history.update({name: [] for name in modeler.metrics.keys()})
        self.history.update({f"val_{name}": [] for name in ['loss'] + list(modeler.metrics.keys())})
    ...
    def fit(self, train_loader, num_epochs, valid_loader=None):
        ...
        self.log(f"Model type: {self.modeler.get_model_type()}")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            total_loss, total_metrics, num_batches = 0.0, {m: 0.0 for m in self.modeler.metrics}, 0
            ...
            with tqdm(train_loader, desc=f"Train {epoch}/{num_epochs}", leave=False) as pbar:
                for batch_data in pbar:
                    batch_results = train_batch(
                        ...
                        batch_data, 
                        self.optimizer,
                        self.modeler.loss_fn, 
                        self.modeler.metrics
                    )
                    total_loss += batch_results['loss']
                    for m in self.modeler.metrics:
                        total_metrics[m] += batch_results[m]
                    num_batches += 1
                    avg_loss = total_loss / num_batches
                    avg_metrics = {m: total_metrics[m]/num_batches for m in self.modeler.metrics}
                    pbar.set_postfix({'loss': f"{avg_loss:.3f}", **{m: f"{v:.3f}" for m, v in avg_metrics.items()}})

            train_results = {'loss': total_loss/num_batches}
            train_results.update({m: total_metrics[m]/num_batches for m in self.modeler.metrics})
            ...
            if valid_loader is not None:
                ...
                valid_results.update({m: total_metrics[m]/num_batches for m in self.modeler.metrics})
                ...
                if self.early_stopping is not None:
                    if self.early_stopping(valid_results['loss'], self.modeler.model):
...
if __name__ == "__main__":
    # Test Trainer with Modeler
    import torch
    from model import get_modeler
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("Testing Trainer with Modeler...")
    ...
    # Create optimizer
    optimizer = get_optimizer(modeler.model, 'adam', lr=1e-3)
    
    # Create scheduler
    scheduler = get_scheduler(optimizer, 'step', step_size=5)
    ...
    print(f"Anomaly trainer created with model: {modeler.get_model_type()}")
    print(f"Metrics: {list(modeler.metrics.keys())}")
    print("Trainer testing completed!")