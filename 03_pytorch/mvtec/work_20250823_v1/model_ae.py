...
class VanillaAE(nn.Module):
    ...
    def train_step(self, batch_data, optimizer, loss_fn, metrics={}):
        """Training step for VanillaAE"""
        self.train()
        ...
        with torch.no_grad():
            for metric_name, metric_fn in metrics.items():
                metric_value = metric_fn(outputs['reconstructed'], batch_data['target'])
                results[metric_name] = metric_value if isinstance(metric_value, (int, float)) else metric_value.item()
        
        return results

    @torch.no_grad()
    def validate_step(self, batch_data, loss_fn, metrics={}):
        """Validation step for VanillaAE"""
        self.eval()
        device = next(self.parameters()).device
...
class VAE(nn.Module):
    ...
    def train_step(self, batch_data, optimizer, loss_fn, metrics={}):
        ...
        loss = loss_fn(outputs['reconstructed'], batch_data['target'], outputs['mu'], outputs['logvar'])
        ...
        with torch.no_grad():
            for metric_name, metric_fn in metrics.items():
                if metric_name == 'vae':
                    metric_value = metric_fn(outputs['reconstructed'], batch_data['target'], outputs['mu'], outputs['logvar'])
                else:
                    metric_value = metric_fn(outputs['reconstructed'], batch_data['target'])
                results[metric_name] = metric_value if isinstance(metric_value, (int, float)) else metric_value.item()
