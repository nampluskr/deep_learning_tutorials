...
class VisADataset(Dataset):
    ...
    def __init__(self, data_dir, categories, split, transform=None, normal_only=False, **kwargs):
        ...
        self.extensions = kwargs.get('extensions', ['*.JPG', '*.jpg'])
        self.normal_folder = kwargs.get('normal_folder', 'good')
        self.anomaly_folder = kwargs.get('anomaly_folder', 'anomaly')
...
class VisADataloader:
    ...
    def __init__(self, data_dir, categories,
        ...
        # Create datasets
        train_dataset = VisADataset(data_dir, categories, 'train', train_transform, **params)
        ...
        if valid_ratio > 0.0:
            ...
            valid_dataset = VisADataset(data_dir, categories, 'train', test_transform, **params)
            self.valid_dataset = Subset(valid_dataset, valid_subset.indices)
        else:
            self.train_dataset = train_dataset
            self.valid_dataset = None

        self.test_dataset = VisADataset(data_dir, categories, 'test', test_transform, **params)
