...
class BTADDataset(Dataset):
    def __init__(self, data_dir, categories, split="train", transform=None, normal_only=False):
        ...
        for cat in self.categories:
            base_path = os.path.join(data_dir, "btad", cat, split)
...
def get_dataloaders(data_dir, category="01", img_size=256, batch_size=8, valid_ratio=0.2, transform=None):
    from torchvision.transforms import v2 as T
    if transform is None:
        transform = T.Compose([T.Resize((img_size, img_size))])

    dataset = BTADDataset(data_dir, categories=[category], split="train", transform=transform, normal_only=True)
    test_dataset = BTADDataset(data_dir, categories=[category], split="test", transform=transform)
