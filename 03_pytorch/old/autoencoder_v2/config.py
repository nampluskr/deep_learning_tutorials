...
class Config:
    ...
    # 모델 설정
    model_type: str = 'vanilla_ae'  # 'vanilla_ae', 'unet_ae', 'vanilla_vae', 'unet_vae', 'resnet_ae', 'vgg_ae', 'efficientnet_ae'
    latent_dim: int = 512
    beta: float = 1.0  # VAE의 KL divergence weight

    # Pretrained 모델 전용 설정 (새로 추가)
    backbone: Optional[str] = None  # 'resnet18', 'resnet50', 'vgg16', 'vgg19', 'efficientnet_b0', 'efficientnet_b4'
    pretrained: bool = True  # ImageNet pretrained weights 사용 여부
    freeze_backbone: bool = False  # Backbone freeze 여부
    ...
    def __post_init__(self):
        if self.experiment_name is None:
            backbone_suffix = f"_{self.backbone}" if self.backbone else ""
            self.experiment_name = f"{self.model_type}{backbone_suffix}_{self.category}_{self.latent_dim}_{self.img_size}"
